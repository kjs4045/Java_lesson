...
//		for문으로
		...
		for(int i=0;i<result.length;i++) {
			System.out.printf(String.format("result[%d] : %s" , i , result[i]));
		}
...
//		향상된for
		...
		for(String t : result) 
			System.out.println("result 요소 : " + t);
		//result[i],result[3],result[5] 3개의 빈 문자열 입니다.
		...
		// 		정규식 예시: []안에 표시하는것은 문자 여러개가 또는 (or)  에 해당됩니다.
		temp2="모모,90 88()79";
		...
		//limit 은 리턴되는 배열의 크기. 단 쪼개지는 갯수보다 커질수는 없음.
		System.out.println("result  length : " + result.length);
		for(String t : result) 
			System.out.println("result 요소 : " + t);
		...
		//위의 rlist는 배열의 크기만큼 List의 크기를 정적으로 생성합니다.
...
//		rlist.add(2, "100");			//오류
		...
		//Arrays.asList 는 테스트용으로 또는 간단한 고정 리스트 사용으로 만들 떄 유용합니다.
