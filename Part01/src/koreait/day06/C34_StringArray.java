...
public class C34_StringArray {
	...
	public static void main(String[] args) {
		...
		//int,double,char 배열에서는 요소를 나타내는 num[2], carr1[2], point[2]
		//                                     ㄴ 기본형 데이터 타입
		//engKey 뿐만 아니라 engKey[2] 도 는 String 참조형 타입
		
		for(int i=0;i<engKey.length;i++)
			System.out.println(engKey[i]); //engKey[i]sms String 참조형
		...
		for(int i=0;i<koreans.length;i++)
			if(koreans[i] != null)//***********************************************************많이 쓰는 if문
			System.out.println(koreans[i]+ ":" + koreans[i].length());
		//참조하고 있는 객체가 없을댸 메소드/필드 참조하면 오류 :koreans[3],koreans[4]는 null을 참조 =오류
...
//        System.out.println(temp.length());
		...
		Scanner sc = new Scanner(System.in);
		 ...
		 for(i=0;i<engKey.length;i++) {
			 ...
			 //break로 for를 중단하지 않고 for를 모두 실행했을때는 같은 문자열이 배열 engKey에 없을 때 입니다.
			 // i=?
