...
//sort와 관련된 인터페이스 사용
	...
	public static void main(String[] args) {
		...
		//다형성: List를 참조타입으로 하여 선언. 앞으로 배울 라이브러리들이 List 타입으로 원합니다.
		...
		ilist.sort(Comparator.reverseOrder());          //reverse:역순, Comparator인터페이스의
...
//		mlist.sort(null);              //오류 : Member 객체는 비교할 수 없는 객체 -> 비교자 인터페이스를 인자로 해야함. comparator객체필요
							//Member 객체의 sort : 비교가능한 값으로 필드 중 하나를 선택해서 비교자(Copmarator) 구현체를 만듭니다.
...
//		mlist.sort(Comparator.reverseOrder());
		...
		ulist.sort(null);
		System.out.println("user sort(ASCE) : " + ulist);
